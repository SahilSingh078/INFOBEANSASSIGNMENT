...
Note: Use filter() to select eligible students, map() to add the bonus marks, and sorted() to display 
the final marks in descending order. All three operations must use lambda expressions.
...
main = list(map(int,input("Enter marks [using space]: \n").split()))[:n]
result = list(filter(lambda x: x>=75, main))
res = list(map(lambda i: i+5, result))
print("Scholarship Marks")
b = sorted(res, key=lambda x: x, reverse =True)
print(*b)